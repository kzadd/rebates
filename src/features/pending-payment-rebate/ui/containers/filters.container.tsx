        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button
              className={cn(
                'justify-start border bg-white text-sm shadow-sm outline-none transition-colors hover:bg-white data-[state=open]:border-primary text-black',
                !paymentDate && 'text-disabled-foreground'
              )}
            >
              <CalendarIcon />
              {paymentDate ? format(paymentDate, 'dd/MM/yyyy') : <span>Ingresa la fecha</span>}
            </Button>
          </PopoverTrigger>

          <PopoverContent align="start" className="w-auto p-0">
            <Calendar initialFocus mode="single" onSelect={setPaymentDate} selected={paymentDate} />
          </PopoverContent>
        </Popover>
      </div>
